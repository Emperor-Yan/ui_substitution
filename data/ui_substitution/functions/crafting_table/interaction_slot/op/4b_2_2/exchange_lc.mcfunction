...
execute on attacker run data modify entity @e[limit=1,distance=..0.001,tag=uis_crafting_table,type=marker] data.material[{Slot:4b}].Item set from entity @s SelectedItem

#将玩家手持物品设为对应的material_display中的物品
execute at @s positioned ~ ~.2 ~ on attacker run item replace entity @s weapon.mainhand from entity @e[limit=1,distance=..0.001,tag=material_display,tag=slot_2_2] container.0

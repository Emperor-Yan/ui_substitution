...
data modify entity @s data.material[{Slot:8b}].Item.Count set from entity @e[limit=1,sort=nearest,tag=material_display,tag=uis_taking_half_D] item.Count

#检查material_display是否为空，若是，则删除
execute at @e[limit=1,sort=nearest,tag=material_display,tag=uis_taking_half_D,nbt={item:{Count:0b}}] run function ui_substitution:crafting_table/interaction_slot/op/8b_3_3/clear

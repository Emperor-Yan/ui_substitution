...
execute at @s positioned ~ ~.2 ~ unless entity @e[limit=1,distance=..0.001,tag=material_display] at @s align xyz run function ui_substitution:crafting_table/interaction_slot/op/0b_1_1/new_slot_lc
...
execute at @s align xyz run data modify entity @e[sort=nearest,limit=1,tag=uis_temp] item set from entity @e[limit=1,distance=..0.001,tag=uis_crafting_table,type=marker] data.material[{Slot:0b}].Item
...
#take_all特判
execute if entity @e[limit=1,distance=..0.001,tag=uis_crafting_table,type=marker,nbt={data:{split:{different_item_s:0,different_player_slot:1},temp:{hold_item_lc:0}}}] run function ui_substitution:crafting_table/interaction_slot/op/0b_1_1/take_all
